...
export async function buildPdf(sheets, name, theme){
  ...
  obj(2, '<< /Type /Pages /Kids ['+[idxId].concat(pageIds).map(id=>id+' 0 R').join(' ')
    +'] /Count '+(sheets.length+1)+' >>');
  obj(3, '<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica /Encoding /WinAnsiEncoding >>');

  // ---- contents: one line per sheet, each line a link to that page ---------
  // A batch of sheets is a document, and a document that cannot be navigated is
  // a pile. The rows are real link annotations, so a click lands on the page -
  // and the numbers still read as a list if the reader ignores them.
  {
    const W=A4[0], H=A4[1];
    const top=H-MARGIN-46;
    const room=top-MARGIN-10;
    const step=Math.min(19, Math.max(11, room/Math.max(1,sheets.length)));
    const fs=Math.min(11, Math.max(7.5, step*0.62));
    const rows=[], annots=[];
    sheets.forEach((s,i)=>{
      const y=top-step*i;
      if(y<MARGIN+8) return;                    // more sheets than page: the rest go unlisted
      const label=(i+1)+'.  '+s.title;
      const num=String(i+2);                    // the contents page is page 1
      rows.push('BT /F1 '+fs.toFixed(1)+' Tf '+type+' rg '+(MARGIN+6).toFixed(2)+' '+y.toFixed(2)
    ...
    const body=enc(
      paper+' rg 0 0 '+W.toFixed(2)+' '+H.toFixed(2)+' re f\n'
      +head+'\n'
      +rule+' RG 0.5 w '+MARGIN.toFixed(2)+' '+(H-MARGIN-32).toFixed(2)+' m '
      +(W-MARGIN).toFixed(2)+' '+(H-MARGIN-32).toFixed(2)+' l S\n'
      +rows.join('\n')+'\n');
  ...
  sheets.forEach((s,i)=>{
    ...
    const foot=(name?name+'  ·  ':'')+s.title+'  ·  '+(i+2)+' / '+(sheets.length+1);
