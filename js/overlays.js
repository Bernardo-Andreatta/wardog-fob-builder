...
// A drawing is taken by its box, the same box the selection draws around it -
// asking for the line itself meant hunting a stroke a pixel or two wide, and
// meant a square or a circle could only be caught on its outline, never inside
// where the shape plainly is. Everything else on the board is taken by the
// ground it covers; this is no different.
export function hitStroke(pt){
  const pad=6/ST.view.scale;
  for(let i=ST.strokes.length-1;i>=0;i--){ const s=ST.strokes[i];
    if(!layerEditable(s)) continue;
    const bb=strokeBBox(s); if(!bb) continue;
    const g=pad + (s.width||0)/2;
    if(pt.x>=bb.minx-g && pt.x<=bb.maxx+g && pt.y>=bb.miny-g && pt.y<=bb.maxy+g) return s;
  }
  return null; }
